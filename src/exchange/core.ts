...
import { updatePairDayData, updateTokenDayData, updateUtradeDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let utrade = UTradeFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  utrade.totalLiquidityBNB = utrade.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  utrade.totalLiquidityBNB = utrade.totalLiquidityBNB.plus(trackedLiquidityBNB);
  utrade.totalLiquidityUSD = utrade.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  utrade.totalTransactions = utrade.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let utrade = UTradeFactory.load(FACTORY_ADDRESS);
  utrade.totalVolumeUSD = utrade.totalVolumeUSD.plus(trackedAmountUSD);
  utrade.totalVolumeBNB = utrade.totalVolumeBNB.plus(trackedAmountBNB);
  utrade.untrackedVolumeUSD = utrade.untrackedVolumeUSD.plus(derivedAmountUSD);
  utrade.totalTransactions = utrade.totalTransactions.plus(ONE_BI);
  ...
  let utradeDayData = updateUtradeDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  utradeDayData.dailyVolumeUSD = utradeDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  utradeDayData.dailyVolumeBNB = utradeDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  utradeDayData.dailyVolumeUntracked = utradeDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  utradeDayData.save();
